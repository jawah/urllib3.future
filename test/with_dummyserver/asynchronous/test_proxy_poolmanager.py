...
class TestAsyncHTTPProxyManager(HTTPDummyProxyTestCase):
    ...
    async def test_https_proxy_with_proxy_ssl_context(self, forwarding: bool) -> None:
    ...
    async def test_https_proxy_keeps_its_verification_policy(
        ...
        async with async_proxy_from_url(
            ...
            for _ in range(2):
                with pytest.raises(MaxRetryError) as exc:
                    await proxy.request("GET", target_url, retries=0)
    ...
    async def test_forwarding_proxy_ignores_destination_identity_and_credentials(
        ...
        async with async_proxy_from_url(
            ...
            response = await proxy.request("GET", self.https_url, retries=0)
    ...
    async def test_cross_host_redirect_remove_headers_via_proxy_manager(
        ...
        async with async_proxy_from_url(self.proxy_url) as proxy_mgr:
            r = await proxy_mgr.request(
            ...
            for header in sensitive_headers:
                ...
                assert header.title() not in data
    ...
    async def test_cross_host_redirect_remove_headers_via_pool(
        self, sensitive_headers: dict[str, str]
    ) -> None:
        headers_url = f"{self.http_url_alt}/headers"
        initial_url = f"{self.http_url}/redirect?target={headers_url}"
        async with async_proxy_from_url(self.proxy_url) as proxy_mgr:
            pool = await proxy_mgr.connection_from_url(self.http_url)
    ...
    async def test_proxy_and_target_tls_verification(
        ...
        async with async_proxy_from_url(
            ...
            if use_forwarding_for_https:
                response = await proxy.request("GET", self.https_url)
                assert response.status == 200
                assert_is_verified(proxy, proxy=True, target=False)
                return
