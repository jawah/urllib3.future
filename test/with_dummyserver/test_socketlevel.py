...
class TestReuseAfterGoaway(SocketDummyServerTestCase):
    ...
    def test_idle_http11_connection_is_not_read_before_reuse(self) -> None:
        # Only multiplexed connections are read before reuse. Reading the 408
        # into the idle HTTP/1.1 connection made the next request raise.
        self._start_server(self._stale_408_handler)

        with HTTPConnectionPool(self.host, self.port, retries=False) as pool:
            assert pool.request("GET", "/").status == 200
            time.sleep(1.2)
            assert pool.request("GET", "/").status == 408
